...
// @desc Create notification (admin - broadcast or single user)
// @route POST /api/notifications/admin
// @access Private/Admin
exports.createNotificationAdmin = async (req, res, next) => {
  try {
    const { type = 'system', title, message, target = 'all', userId, link } =
      req.body;
    ...
    if (target === 'all') {
      // You can either:
      // 1) insert many notifications, or
      // 2) store a single "system" notification and show it to all (needs frontend support)
      //
      // Here we'll do per-user notifications for clarity.
    ...
    if (!userId) {
      return res.status(400).json({
        success: false,
        message: 'userId is required when target is not "all"',
      });
    }
